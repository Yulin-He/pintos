...
void test_priority_donate_one(void)
{
  ...
  lock_init(&lock);
  lock_acquire(&lock);                                                     // 主线程持有锁lock
  thread_create("acquire1", PRI_DEFAULT + 1, acquire1_thread_func, &lock); // 让线程a得到锁
  msg("This thread should have priority %d.  Actual priority: %d.",
      PRI_DEFAULT + 1, thread_get_priority());
  thread_create("acquire2", PRI_DEFAULT + 2, acquire2_thread_func, &lock); // 让线程b得到锁
  msg("This thread should have priority %d.  Actual priority: %d.",
...
acquire1_thread_func(void *lock_)
{
  ...
  lock_acquire(lock); // 锁不可用（已被主线程占用），阻塞线程acquire1，放入等待队列
...
acquire2_thread_func(void *lock_)
{
  ...
  lock_acquire(lock); // 锁不可用（已被主线程占用），阻塞线程acquire2，放入等待队列
