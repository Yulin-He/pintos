...
{
  struct lock_and_sema ls; // 包含锁和信号量
  ...
  sema_init(&ls.sema, 0);                                     // 初始化信号量
  thread_create("low", PRI_DEFAULT + 1, l_thread_func, &ls);  // 创建low
  thread_create("med", PRI_DEFAULT + 3, m_thread_func, &ls);  // 创建med
  thread_create("high", PRI_DEFAULT + 5, h_thread_func, &ls); // 创建high
  sema_up(&ls.sema);                                          // V操作，low执行，释放锁，high执行，med执行
...
l_thread_func(void *ls_)
{
  ...
  lock_acquire(&ls->lock); // 获取锁，继续执行
  msg("Thread L acquired lock.");
  sema_down(&ls->sema); // P操作，阻塞
...
h_thread_func(void *ls_)
{
  ...
  lock_acquire(&ls->lock); // 获取锁，阻塞
  msg("Thread H acquired lock.");

  sema_up(&ls->sema);      // V操作
  lock_release(&ls->lock); // 释放锁
